...
class AdBannerRequest(BaseModel): 
    ...
    product_photo_url_banner: str  
    reference_photo_url_banner: str
    input_details_banner: str
...
@app.post("/refine-adbanner/")
async def refine_banner(request: AdBannerRequest):
    # THESE WERE HERE EARLIER
    product_photo_url_banner = request.product_photo_url_banner
    reference_photo_url_banner = request.reference_photo_url_banner
    input_details_banner = request.input_details_banner

    print(f"Received product photo URL of banner: {product_photo_url_banner}")
    print(f"Received reference photo URL of banner: {reference_photo_url_banner}")
    print(f"Received input details of banner: {input_details_banner}")



    try:
        # Convert image urls to base64 THIS WAS HERE EARLIER, because we need to send the base64 format to got
        product_photo_adbanner_base64 = image_to_base64(product_photo_url_banner) if product_photo_url_banner else None
        reference_photo_adbanner_base64 = image_to_base64(reference_photo_url_banner) if reference_photo_url_banner else None

        # Prepare the messages list
        messages = []

        # Logic when both product photo and reference photo are present //THIS WAS EARLIER
        if product_photo_url_banner and reference_photo_url_banner:  
            messages.append({
                "role": "user",
                "content": [
                    {
                        ...
                        "text": (
                              ...
                              #f"Don't mention a single thing about the product in the prompt, not even the word 'product'. This is because we use inpainting so you should only give prompt that defines everything other than the product. The product will always be positioned at the center of the ad-creative so the elements of the ad creative should be around the centre. Provide a visually balanced design suited to the product type and theme."
                              f"Make the prompt into 2 paragraphs. The first paragraph should describe the 'top of the ad text', 'OFF badge or text' elements of ad creative and the other paragraph should describe 'Highlight note text' and 'Call to action text' elements."
                              f"The prompt should always start with 'Design a background layout for an ad-creative...' and continue."
                              f"Refer to any text elements as 'text' (e.g., 'The top of the ad should say, xyzxyz...') rather than using terms like 'title' or 'bullet points.' Describe the position, style, text font, font size of all elements explicitly."
                              f"The prompt should be maximum of 120 words."



                            #   f"You are an expert prompt generator specializing in creating high-quality, visually appealing  advertisement layout for professional use. Below are two images: the first is the product photo, and the second is the reference photo. Use the reference photo as inspiration to design the ad-banner for the product provided. The prompt you generate will be sent to the Flux image generation model to create the ad-banner. Ensure the generated banner is of significantly higher quality than the reference photo."
                            #   f"There are the adbanner details that needs to be generated in the banner, {input_details_banner}."
                            #   f"The prompt should only describe the ** advertisement layout**, **design elements**, **creative assets**, **color**, **shades** and **lightning**—avoid describing the product itself. This is because we use inpainting, and the product will always be positioned at the center of the banner. Provide a visually balanced design suited to the product type and theme."
                            #   f"Ensure the prompt is clear and concise, written in 2 paragraphs. The first paragraph should cover half features of the banner and the second paragraph should cover other half."
                            #   f"Refer to any text elements as 'text' (e.g., 'Add a text at the top saying...') rather than using terms like 'title' or 'bullet points.' Describe the position, style, text font, font size of all elements explicitly."
                            #   f"Strictly ensure that all texts and details provided by the user are displayed in the ad-banner **without any typos** or errors. If there are unclear or excessively small text details, omit them from the prompt."
                            #   f"Keep the prompt length balanced—not too long or too short—and avoid clutter in the banner design. Aim for a professional, clean, and aesthetically pleasing result."
                    ...
                    {   
                         "type": "image_url",  # Send the base64 encoded image in place of the URL
                            "image_url": {
                                "url": f"data:image/png;base64,{product_photo_adbanner_base64}"
                            }
                    },
                    {
                        "type": "image_url",
                            "image_url": {
                                "url": f"data:image/png;base64,{reference_photo_adbanner_base64}"
                            }
                    }
                ]
            })
        ...
        # elif reference_photo_url_banner: #THIS WAS EARLIER
        #     messages.append({
        #         "role": "user",
        #         "content": [
        #             {
        #                 "type": "text",
        #                 "text": (
        #                     f"You are an expert prompt generator specializing in high-quality, studio-level product photography background setup, the prompt you give will be sent to flux image generation model to generate the image. Give me a prompt that generates an image much better than this below given reference image qulaity"
        #                     f"Dont give prompt for product, give only the background and environment setup, because we will use inpainting so you should only give prompt that defines everything other than the product like lightning, color, shade, setup and design elements in backdrop, anything that suits the product, product will be positioned at centre."                            
        #                     f"Dont leave spaces inbetween lines in prompt"
        #                     f"Consider the input details in the prompt given by the user, {input_details_banner}."
                ...
                "content": [
                    {
                        ...
                        "text": (
                          ...
                          f"Refer to any text elements as 'text' (e.g., 'The top of the ad should say, xyzxyz...') rather than using terms like 'title' or 'bullet points.' Describe the position, style, text font, font size of all elements explicitly."
                          f"The prompt should be maximum of 120 words."
                          #f"Refer to any text elements as 'text' (e.g., 'At the top of ad creative, add text saying, xyzxyz...') rather than using terms like 'title' or 'bullet points.' Describe the position, style, text font, font size of all elements explicitly."


                          
                          #f"Aim for a professional, clean, and aesthetically pleasing result. The prompt should be maximum 115 words."
                        ...
                        #   f"Dont leave spaces inbetween lines in prompt"
                        #   f"Consider the input details in the prompt given by the user, {input_details_banner}."
                        #   f"Dont tell in the prompt to 'generate something as provided in the photo', because flux model can't see the image so you mention clearly what needs to be generated, flux model just generates according to the prompt you give so they has to be specific words and not like some description to a person "
                        #     # f"If there are too many small unclear texts, then omit them from prompt"
        ...
        generated_banner_url = generate_product_photo(gpt4o_result, product_photo_url_banner)
        #generated_banner_url = "blob:https://web.whatsapp.com/3971026b-3f1e-43c7-b5e6-8cf401f5d96a"

        if generated_banner_url:
            print(f"COmfyOnline Generated Url: {generated_banner_url}")   
        else:
            print("Failed to generate product photography.")    

        return {"gpt4o_result": gpt4o_result, "Generated_AdBanner_URL": generated_banner_url} #HERE JUST ADD replicate url
        #Till here
...
async def process_images(request: PhotographyRequest):
    ...
    try:
        ...
        #Earlier this below snippet was used, it gets product photo file name, uploads the pic to S3 and returns that url
        ...
        #From here to below it was added instead of above snippet
        generated_product_photo_url = generate_product_photo(gpt4o_result, product_photo_url)

        if generated_product_photo_url:
            print(f"COmfyOnline Generated Url: {generated_product_photo_url}")   
        else:
            print("Failed to generate product photography.")    

        return {"gpt4o_result": gpt4o_result, "Generated_Product_Photo_URL": generated_product_photo_url} #HERE JUST ADD replicate url
        #Till here
...
def generate_product_photo(image_prompt, product_photo_url):
    try:
        ...
        payload = {
            "input": {
                "CLIPTextEncode_text_4": image_prompt,  
                "LoadImage_image_7": product_photo_url,  
                "CLIPTextEncode_text_13": "nsfw, worst, blur, poor quality"
            },
            "workflow_id": "4e6126ad-b230-49e4-9e64-9da7471144c0",  
        }
        ...
        for _ in range(150):  # Retry for ~30 seconds
            ...
            status_response = requests.post(
                f"https://api.comfyonline.app/api/query_run_workflow_status",
                headers=headers,
            ...
            print(f"Task status response: {status_json}")
              ...
              # Extract state and output fields
            state = status_json.get("data", {}).get("state")
            output = status_json.get("data", {}).get("output", {})

            if state == "COMPLETED":
                output_url_list = output.get("output_url_list", [])
                if output_url_list:
                   print(f"Task completed. Output image(s): {output_url_list}")
                   break
                else:
                    print("Task completed but no output image URLs found.")
                    raise Exception("Output is empty despite task completion")
            elif state == "ERROR":
               print("Task failed with error status.")
               raise Exception("Task failed with error status")
            else:
               print(f"Current task status: {state}")

            time.sleep(2)   
        else:
            print("Task polling timeout. Task status could not be retrieved.")
            ...
            #     output_url_list = status_json.get("data", {}).get("output", {}).get("output_url_list")
